...
'@
        ...
        "putty" {
            try {
                # Precreate PuTTY registry hive to avoid any initial prompts (best effort)
                $puttyKey = "HKCU:\Software\SimonTatham\PuTTY"
                Ensure-RegistryPath $puttyKey
                New-ItemProperty -Path $puttyKey -Name "WarnOnClose" -Value 0 -PropertyType DWord -Force -ErrorAction SilentlyContinue | Out-Null
                Write-Host "PuTTY first-run suppression configured (best effort)" -ForegroundColor Green
            } catch {
...
'@
        ...
        "7zip" {
            try {
                # 7-Zip typically has no first-run dialogs; create optional config directory for completeness
                $sevenDir = "$env:APPDATA\7-Zip"
                if (!(Test-Path $sevenDir)) { New-Item -Path $sevenDir -ItemType Directory